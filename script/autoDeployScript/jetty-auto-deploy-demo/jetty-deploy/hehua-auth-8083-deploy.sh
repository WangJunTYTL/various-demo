...
CONTROL_SCRIPT=/Users/wangjun/jetty-auto-deploy-demo/jetty-control/hehua-auth-8083-control.sh # 控制脚本位置
EMPTY=''

#初始化
[ "x$PROJECT_NAME" == "x" ] && echo "项目名为空"&& exit 1
[ "x$BRANCH" == "x" ] && echo "项目分支为空"&& exit 1
[ "x$ENVIRONMENT" == "x" ] && echo "mvn 编译要求的 -P 参数为空"&& exit 1
...
#同步构建后的文件到正式目录
rsync -ztrlvC --delete ${CODE_DOWN}/web/target/${PROJECT_NAME}*/ ${CODE_BUILD}/${PROJECT_NAME}
valid $? "同步失败"
