...
def send_mail(subject, content, address_from, address_to, smtp_host, smtp_user, smtp_password, smtp_port=25,
              using_tls=False):
    import smtplib
    ...
    if is_gmail:
        smtp_port = 587
    print("smtp_host = %s smtp_port = %s" % (smtp_host, smtp_port))
    smtp.connect(smtp_host, smtp_port)
    if using_tls or is_gmail:
        print('smtp.starttls()')
        smtp.starttls()
    if smtp_user:
        print ("smtp_user = %s  smtp_password = %s" % (smtp_user, smtp_password))
...
if __name__ == "__main__":
    ...
    parser.add_option('-f', '--from', dest='address_from', metavar='my@domain.com',
                      help='Set envelope from address. If --user option is not empty, commonly this option should be equaled with --user options. Otherwize, the authoration of the smtp server should be failed.')
    ...
    parser.add_option('-F', '--file', dest='file',
                      help='Read mail body from file. NOTE:  --file will be ignored if work with --content option at same tome.')
    parser.add_option('--host', metavar='smtp.domain.com',
                      help='SMTP server host name or ip. Like "smtp.gmail.com" through GMail(tm) or "192.168.0.99" through your own smtp server.')
    parser.add_option('-P', '--port', type='int', default=25, help='SMTP server port number. Default is %default.')
    parser.add_option('-u', '--user', metavar='my@domain.com',
                      help='The username for SMTP server authorcation. Left this option empty for non-auth smtp server.')
    parser.add_option('-p', '--password',
                      help='The password for SMTP server authorcation. NOTE: if --user option is empty, this option will be ignored.')
    parser.add_option('--tls', action='store_true',
                      help='Using tls to communicate with SMTP server. Default is false. NOTE: if --host option equals "smtp.gmail.com", this option becomes defaults true.')
    ...
    # title = u"2013大街网组织气氛测评项目启动，欢迎您的参与！"
    # title = u"2013大街网组织气氛测评提醒邮件（重要）：您还未完成测评，请务必在12月16(本周五)前完成！"
    title = opts.subject
    send_mail(title, content, opts.address_from, opts.address_to, opts.host, opts.user, opts.password, opts.port,
              opts.tls)
