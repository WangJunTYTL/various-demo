...
cursor = conn.cursor()
sql = "create table if not exists user(id int ,name varchar(8))"
cursor.execute(sql)
sql = "insert into user (id,name) values(1,'wj')"
cursor.execute(sql)
conn.commit()
...
for row in cursor.fetchall():
    print("%s\t\t%s" % row)
print("table user has %s rows" % cursor.rowcount)  # sqlite不支持
sys.exit(0)
